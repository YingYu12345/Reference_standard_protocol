...
print_usage <- function() {
  cat("Actual Usage: Rscript stability_assess.R [options]\n\n")
  cat("Example:\n")
  cat("  Rscript path-to/stability_assess.R -i path-to/example_expr_stability_RIN.csv -m metadata_stability_RIN.csv -o path-to/\n")
...
expr <- read.csv(opt$input, header = TRUE, stringsAsFactors = FALSE, row.names = 1, check.names = FALSE)
...
t95 <- data.frame(
  N = 1:25,
  t95 = c(12.706, 4.303, 3.182, 2.776, 2.571, 2.447, 2.365, 2.306, 2.262, 2.228,
...
RINvalue_m$time <- sapply(strsplit(rownames(RINvalue_m), " "), `[`, 1)
RINvalue_m$sample <- sapply(strsplit(rownames(RINvalue_m), " "), `[`, 2)
...
unumsample <- usample[-1]

RINvalue_ratio <- data.frame()
...
# Output
stabilityfile <- paste0(out_dir, "stability_lm_detail_", Sys.Date(), ".csv")
write.csv(stability, stabilityfile, row.names = FALSE)

message("Finished!")
message(paste("Output file: ", stabilityfile))